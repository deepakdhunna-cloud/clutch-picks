...
const MATCHUP_CARD_CONTENT_PADDING_Y = 24;
...
const MATCHUP_CARD_MIN_HEIGHT = 178;
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
          ...
          <View style={{ flex: 1, minWidth: 0, paddingRight: 12 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10, marginBottom: 4 }}>
              <Text style={{ fontSize: 14, lineHeight: 18, fontWeight: '900', color: MATCHUP_ACCENT_COLOR, letterSpacing: 0.2 }}>{startTime}</Text>
              <Text style={{ fontSize: 13, lineHeight: 18, fontWeight: '500', color: TEXT_MUTED, marginHorizontal: 6 }}>{'·'}</Text>
              <Text style={{ fontSize: 12.5, lineHeight: 18, fontWeight: '600', color: TEXT_SECONDARY, flex: 1 }} numberOfLines={expanded ? undefined : 1}>{headline}</Text>
            </View>
            {tags.length > 0 ? (
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: MATCHUP_TAG_ROW_GAP, marginTop: 14 }}>
                {tags.slice(0, expanded ? tags.length : 2).map((tg, i) => (
                  <View key={`${tg}-${i}`} style={{ backgroundColor: MATCHUP_CHIP_BACKGROUND, borderRadius: 7, borderWidth: 1, borderColor: MATCHUP_CHIP_BORDER, paddingHorizontal: 11, paddingVertical: 8 }}>
